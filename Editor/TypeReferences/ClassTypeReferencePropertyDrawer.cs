...
namespace StephanHooft.TypeReferences.EditorScripts
{
	/// <summary>
	...
	public sealed class ClassTypeReferencePropertyDrawer : PropertyDrawer
	{
		...
		/// A value of <c>null</c> specifies that no types are to be excluded.
		/// </summary>
		/// <remarks>
		/// <para>This property must be set immediately before presenting a class type reference property field
		/// using <see cref="EditorGUI.PropertyField"/> or <see cref="EditorGUILayout.PropertyField"/> since the
		/// value of this property is reset to <c>null</c> each time the control is drawn.</para>
		/// <para>Since filtering makes extensive use of <see cref="ICollection{Type}.Contains"/> it is recommended
